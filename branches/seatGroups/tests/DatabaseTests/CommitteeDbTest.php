...
class CommitteeDbTest extends PHPUnit_Framework_TestCase
{
	...
	public function testGetSeatGroups()
	{
    	$committee = new Committee();
    	$committee->setName('test committee');
		$committee->save();
		...
		{
			$this->assertTrue($committee->getSeatGroups() instanceof SeatGroupList);
			foreach($committee->getSeatGroups() as $group)
			{
				$this->assertEquals($group->getCommittee_id(),$committee->getId());
			}
		}
	}
    ...
    public function testGetSeats()
    {
    	...
    	foreach($list as $committee)
    	{
    		...
    		{
				$this->assertEquals($seat->getCommittee()->getId(),$committee->getId());
    		}
    	}
    }
