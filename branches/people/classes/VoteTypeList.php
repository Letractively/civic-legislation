<?php
/**
 * A collection class for VoteType objects
 *
 ...
 */
class VoteTypeList extends PDOResultIterator
{
	/**
	...
	{
		$this->select = 'select voteTypes.id as id from voteTypes';
		if (is_array($fields)) {
			$this->find($fields);
		}
	}
	...
	/**
	 * Loads a single VoteType object for the key returned from PDOResultIterator
	 * @param int $key
	 */
	protected function loadResult($key)
	{
		return new VoteType($this->list[$key]);
	}
}
